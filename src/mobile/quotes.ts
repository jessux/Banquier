 ...
 *
 * Port de src/main/quotes.ts, simplifié pour mobile : `capacitor.config.ts` active
 * `CapacitorHttp`, qui route le `fetch()` de la WebView nativement côté Android
 * (même mécanisme que src/mobile/llm.ts pour les appels OpenRouter) — ça évite les
 * blocages CORS d'un appel direct à une API tierce depuis une WebView, sans avoir
 * besoin de `node-fetch`. Et comme Android n'a pas l'équivalent d'un proxy
 * d'entreprise à configurer manuellement (contrairement à desktop, cf.
 * `setConfiguredProxy` dans src/main/index.ts), l'agent HTTPS du port desktop
 * (`https-proxy-agent`) disparaît aussi : on utilise directement le `fetch()`
 * global.
 */
...
/** Point de la série de cours historiques (ts en secondes, prix en €). */
