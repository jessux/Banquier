...
- `src/mobile/parsers/pdf.ts` — extraction des fragments de texte PDF avec positions (`pdfjs-dist`, côté client), parsés par la logique partagée `src/shared/pdfBankStatement.ts` (voir Phase 5)
...
L'import PDF est un parseur de tableau 100 % local (aucun appel réseau, aucun LLM) : la logique vit dans `src/shared/pdfBankStatement.ts` (fonctions pures, sans dépendance Node ni navigateur), partagée telle quelle entre desktop et mobile — seule l'étape d'extraction des fragments de texte du PDF (avec leurs coordonnées x/y) diffère selon la plateforme.

`extractTransactionsFromPages` reconstruit le tableau Date/Nature/Valeur/Débit/Crédit à partir des positions des fragments de texte plutôt que du texte brut : `pdf-parse` et `pdfjs-dist` ne réinsèrent pas fidèlement les espaces entre mots (deux fragments adjacents sans espace réel dans le flux du PDF se retrouvent collés), donc le texte à plat n'est pas exploitable pour un parsing structurel fiable. Le regroupement en lignes/mots par coordonnées, la détection des colonnes Débit/Crédit (ou Montant) par l'en-tête, et la résolution de l'année à partir de la période du relevé sont documentés en commentaires dans ce fichier.

- **Extraction de texte côté client.** Desktop utilise `pdf-parse` (`src/main/parsers/pdf.ts`), un module Node-only (`fs.readFileSync` + un hook `pagerender` custom qui capture `item.transform`/`item.width` par fragment), indisponible dans le bundle renderer mobile (build navigateur classique, cf. `electron.vite.config.ts`). Le portage (`src/mobile/parsers/pdf.ts`) utilise [`pdfjs-dist`](https://github.com/mozilla/pdf.js) (le moteur PDF.js de Mozilla, pur JS/WASM) à la place : le PDF est lu depuis le cache du sélecteur de fichier (`file-picker.ts`, même mécanisme que l'import CSV — `openFileDialog()` retourne un handle opaque, `getCachedFile(handle).base64` donne le contenu), décodé en `Uint8Array` (`atob` + boucle, comme le `decodeBase64ToText` du CSV), puis passé à `pdfjsLib.getDocument({ data })`. Chaque page expose ses fragments (`str`/`transform`/`width`) via `getTextContent()`, convertis vers le même type `PdfTextItem` que desktop puis passés à `extractTransactionsFromPages`.
- **Aucune duplication de logique.** Contrairement à l'ancien flux basé sur un LLM (`buildPdfParsePrompt`/`parseLlmJsonResponse` étaient recopiées verbatim, desktop → mobile, car elles vivaient dans un fichier important `fs`/`pdf-parse`), la logique de parsing du tableau n'a aucune dépendance Node ni navigateur : elle vit dans `src/shared/pdfBankStatement.ts` et est importée directement par les deux plateformes, sans copie à maintenir en synchro.

Un relevé dont la mise en page ne comporte ni en-tête Débit/Crédit (ou équivalent) ni Montant renvoie simplement une liste vide (`errors: 1` côté UI) plutôt qu'un résultat inventé — testé et calé sur deux relevés réels de mises en page très différentes (BNP Paribas et Revolut, voir l'en-tête de `src/shared/pdfBankStatement.ts` pour le détail des différences couvertes). La détection des colonnes est dynamique, donc la plupart des banques françaises devraient être reconnues, mais rien au-delà de ces deux échantillons n'a été vérifié.
...
**Non vérifié.** Comme le reste des flux mobiles marqués « non testée en conditions réelles », l'ensemble (sélection du PDF, extraction de texte par pdf.js en thread principal, parsing du tableau, insertion) n'a été validé que par `npm run typecheck:mobile` et `npm run build:android` (bundling Vite réussi) — jamais dans une vraie WebView Android. Le point le plus susceptible de mal se comporter sur un appareil réel reste ce comportement de repli de pdf.js en l'absence de Worker.
